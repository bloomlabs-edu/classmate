...
Phase 2 builds the complete **teacher** side of Learning Record: creating the syllabus (Subjects → Units → Concepts) and marking concepts taught/not-taught. Per the brief, this phase deliberately does **not** include any student-facing UI, any Learning Hub integration, or any analytics beyond the plain "X of Y taught" count Phase 1 already exposed.

### What Was Built
- **New screen**: `ui/views/LearningRecordView.js` — a three-level drill-down (Subjects → that subject's Units → that unit's Concepts) sharing one route, the same "one file, level decided by which route params are present" shape `NotebookRegisterView.js`/`NotebookTimelineView.js` already use. Add/Edit/Delete at every level; a taught/not-taught toggle only at the Concept level, since a Subject or Unit has no status of its own.
- **New route**: `#/classroom/{id}/learning-record/{subjectId?}/{unitId?}` (see `ui/router.js`).
- **New Dashboard entry point**: a "Learning Record" shortcut chip in the Teaching section (`ui/views/DashboardView.js`), alongside the existing Activities shortcut.
- Every mutation goes through `learningRecordTeacherService.js` only — the view never touches `learningRecordStudentService.js`, keeping Phase 1's structural teacher/student boundary intact.
- Same mutate → `workspaceService.save(classroom)` → rerender convention as `SettingsView.js`/`SetupWizardView.js`; no new persistence work needed (per Phase 1's design, this all rides on the classroom document already being saved as a whole).

### A Deliberate Behavior Change Along the Way
The Dashboard's existing "Teaching" section (Subjects widget + Activities link) was previously hidden entirely until a teacher had configured Notebook Tracker subjects (`hasSubjectsConfigured`). Learning Record is explicitly independent of Notebook Tracker (see `docs/LEARNING_RECORD.md`), so gating its entry point on that same condition would have been an incorrect coupling. Restructured so the Teaching section always renders once a classroom has students, with the Learning Record link always present and the Subjects widget/Activities link joining it only once notebook subjects exist — one section, not two, since two separate `createTeachingSectionElement()` calls would have stacked two identical "Teaching" headings.

### Files Created
- `js/ui/views/LearningRecordView.js`

### Files Modified
- `js/ui/router.js` — new route.
- `js/main.js` — new import, route dispatch, and `onOpenLearningRecord` wiring for the Dashboard.
- `js/ui/views/DashboardView.js` — new Teaching-section entry point; restructured that section's gating (see above).
- `css/styles.css` — taught/not-taught toggle styling (reuses the existing green "complete" semantic already used for buckets/notebooks, not a new color).

### Breaking Changes
None to existing data or behavior. The Teaching-section restructuring changes *when* that section appears (now always, once there are students, instead of only once notebook subjects are configured) but doesn't remove or alter any existing widget's own behavior.

### Regression Verification
No browser/network access in this environment, so verification here is code-level, not live: every touched/created file passed `node --check`; every new relative import path was individually resolved against the actual filesystem rather than trusted on sight (`node --check` doesn't catch a broken import path or a missing function parameter — see the `onSelectStudent` crash earlier in this changelog for exactly that failure mode); every `learningRecordService`/`learningRecordTeacherService` function called from the new view was cross-checked against that service's actual exports. **Live click-through in a real browser (creating a subject, drilling into units/concepts, toggling taught status, deleting at each level, and confirming the Dashboard entry point renders correctly both with and without notebook subjects configured) has not been done and should happen before relying on this.**

### Future TODOs
- (Carried over, unchanged): restructure `hasJoinedPortal` to a top-level field so a Firestore rule can safely permit that write; fix `awardStar()` not reading the classroom's own "Default point value" setting; apply category badges to Settings' own tabs and Activities if a future pass wants full palette coverage; resume broader Settings redesign feature work; Phase 2 activity-state recommendations; mobile-viewport testing as standard practice; Student Workspace tab expansion; note-undo gap in `classModeService`; Session Lock and Session History; role-based routing; confirm whether GitHub Pages is still enabled on the repo and disable it if so; all previously-listed items.
- New, from this entry: Learning Record Phase 3 (student UI — self-reporting understanding, requesting help, per `learningRecordStudentService.js`) and Phase 4 (teacher-facing notebook-status setting per student/concept, and any analytics/reporting beyond the basic taught-count) are both explicitly out of scope for this phase and still to come. Live browser verification of this phase (see above) is also still outstanding.

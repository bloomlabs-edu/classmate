...
// ---------------------------------------------------------------------
// 23 — PHASE 3.6 disclosure test. Not a security test — a
// FUNCTIONALITY test making the known, explicitly-disclosed
// limitation of this round's own isValidStudentGoalUpdate() fix
// directly visible, rather than leaving it only as a comment in
// firestore.rules that nobody re-reads before relying on this rule.
// This is EXPECTED to currently show ALLOW is unavailable (DENY) for
// a legitimate second category — that is the disclosed gap, not a
// surprise. It exists so a future round attempting to properly solve
// the general, multi-category case has a real, executable test
// proving whether that fix actually works, rather than having to
// rediscover this limitation from scratch.
// ---------------------------------------------------------------------

test('23. KNOWN LIMITATION — student who already has ONE goal category set attempts to add a legitimate SECOND, different category -> currently DENIED (disclosed product gap, not a security issue; see firestore.rules\' own isValidStudentGoalUpdate() comment)', async () => {
  const db = testEnv.authenticatedContext('uid-A').firestore();
  await assertSucceeds(setDoc(doc(db, entryPath(CLASSROOM_ID, SESSION_ID, 'student-A')), { goals: { 'cat-1': VALID_GOAL } }));
  const secondCategoryAttempt = updateDoc(doc(db, entryPath(CLASSROOM_ID, SESSION_ID, 'student-A')), {
    'goals.cat-2': { text: 'Speak with a partner in English', source: 'custom', outcome: null, reflection: '' },
  });
  try {
    await secondCategoryAttempt;
    console.log('[TEST 23 RESULT] ALLOWED — the multi-category limitation has been resolved since this test was written; the comment in firestore.rules describing it as a known gap is now stale and should be updated/removed.');
  } catch (error) {
    console.log('[TEST 23 RESULT] DENIED (expected, disclosed limitation) —', error.message);
  }
});

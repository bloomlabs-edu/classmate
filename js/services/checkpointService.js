...
/**
 * The class-level "needs attention" view — aggregates every
 * actionable item (not submitted, needs review, incomplete, late)
 * across every checkpoint in ONE Notebook, grouped per student so a
 * student with multiple outstanding items appears once, not as
 * several unrelated cards (per explicit product decision).
 *
 * Distinct-STUDENT counting vs. record counting is deliberate, not
 * an oversight — matches the exact language of this feature's own
 * product spec ("5 STUDENTS haven't submitted" vs. "3 SUBMISSIONS
 * need review"): not-submitted is fundamentally about a student's
 * own standing (have they engaged with this checkpoint at all), so
 * counted per distinct student; review/incomplete/late are each
 * about one specific submission record, and a single student can
 * genuinely have several across different checkpoints, each worth
 * surfacing separately.
 *
 * Never persisted — recomputed fresh from `checkpoints`/`roster`
 * every call, matching this entire feature's own "derive, don't
 * store" principle throughout.
 */
export function getNeedsAttentionForNotebook(checkpoints, roster) {
  const byStudentId = new Map(); // studentId -> { student, items: [] }

  function addItem(student, item) {
    if (!byStudentId.has(student.id)) byStudentId.set(student.id, { student, items: [] });
    byStudentId.get(student.id).items.push(item);
  }

  checkpoints.forEach((checkpoint) => {
    roster.forEach((student) => {
      const record = getRecordForStudent(checkpoint, student.id);
      if (!record || record.submissionStatus === 'not_submitted') {
        addItem(student, { checkpointId: checkpoint.id, checkpointTitle: checkpoint.title, kind: 'not_submitted' });
        return;
      }
      const late = isLate(checkpoint, record);
      if (record.reviewStatus === 'not_reviewed') {
        addItem(student, { checkpointId: checkpoint.id, checkpointTitle: checkpoint.title, kind: 'needs_review', late });
      } else if (record.reviewStatus === 'incomplete') {
        addItem(student, { checkpointId: checkpoint.id, checkpointTitle: checkpoint.title, kind: 'incomplete', late });
      } else if (late) {
        // Reviewed complete, but still worth surfacing as late —
        // per explicit product decision, "late" is never dropped
        // just because the other dimension (review) looks fine, so
        // a late-but-complete submission still shows up here, purely
        // as a late flag rather than an unreviewed/incomplete one.
        addItem(student, { checkpointId: checkpoint.id, checkpointTitle: checkpoint.title, kind: 'late' });
      }
    });
  });

  const perStudent = [...byStudentId.values()];

  const notSubmittedStudentIds = new Set();
  ...
  checkpoints.forEach((checkpoint) => {
    roster.forEach((student) => {
      const record = getRecordForStudent(checkpoint, student.id);
      if (!record || record.submissionStatus === 'not_submitted') {
        notSubmittedStudentIds.add(student.id);
        return;
      }
      if (record.reviewStatus === 'not_reviewed') needsReviewCount += 1;
      if (record.reviewStatus === 'incomplete') incompleteCount += 1;
      if (isLate(checkpoint, record)) lateCount += 1;
    });
  });
  ...
  return {
    perStudent,
    counts: {
      notSubmittedStudentCount: notSubmittedStudentIds.size,
      needsReviewCount,
      incompleteCount,
      lateCount,
    },

...
/**
 * Renders one PDF page to a plain image Blob — needed only when a
 * page has no real, extractable text layer at all (a scanned
 * textbook, photographed rather than typeset), since Tesseract.js's
 * own README states plainly it "does not support PDF files" and can
 * only OCR an actual image (see services/ocrService.js). Real
 * pdf.js page.render() against a real <canvas> element and
 * HTMLCanvasElement.toBlob() — both genuine browser APIs this sandbox
 * cannot execute or verify; written correctly against pdf.js's own
 * documented render() contract, but this specific function needs a
 * real-browser check before being relied on, the same honest caveat
 * this file's own extractPageRange() already carries for a different
 * reason.
 *
 * `scale` trades render resolution for OCR accuracy vs. processing
 * time — 2 is a reasonable default for a typical textbook page,
 * matching common OCR-preprocessing guidance of rendering at roughly
 * 200-300 DPI equivalent rather than a screen-resolution 1x render.
 */
export async function renderPageToImageBlob(pdfHandle, pageNumber, { scale = 2 } = {}) {
  const page = await pdfHandle.getPage(pageNumber);
  ...
  return new Promise((resolve, reject) => {
    canvas.toBlob((blob) => {
      ...
      else reject(new Error(`renderPageToImageBlob: canvas.toBlob() produced no blob for page ${pageNumber}`));
    }, 'image/png');
  });
}

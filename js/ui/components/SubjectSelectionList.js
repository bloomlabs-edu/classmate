 ...
 * selectable list of suggested subject names, "Custom Subject" as the
 ...
 * imports config/commonSubjectsConfig.js — the deliberate other half
 * of the safeguard described in ui/components/AddSubjectModal.js's own
 * header comment. Only ever rendered from inside that modal's own
 * Choose Subject step; never imported by
 * ui/views/LearningManagementView.js or
...
export function renderSubjectSelectionList({ existingSubjectTitles, onSelect }) {
  ...
  availableSubjects.forEach((subjectName) => {
    ...
    // step for this path.
    row.addEventListener('click', () => onSelect(subjectName));
  ...
  customInput.addEventListener('keydown', (event) => {
    ...
    if (typedName) onSelect(typedName);
  });

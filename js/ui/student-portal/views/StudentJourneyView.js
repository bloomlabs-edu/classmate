 ...
 * services/studentPortalDataService.js's own getWeeklyNetPoints(),
 * which derives every value from the exact same canonical
 * getStarsInRange() every ranking in this app already uses — no new
 * calculation, no interpretation happens here).
 *
 ...
 * heading, a one-line subtitle, and each day's own single-letter
 * initial — no per-point value labels, no legend, no streak.
 */
function createWeeklyNetPointsSection(weeklyNetPoints) {
  ...
  svg.innerHTML = `
    <svg viewBox="0 0 ${width} ${height}" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Weekly net point movement, Monday through Friday">
      ...
      ${points
        ...
        .join('')}
      ${points.map((p) => `<text x="${p.x}" y="${height - 2}" class="weekly-net-graph__day-label" text-anchor="middle">${p.dayLabel[0]}</text>`).join('')}
    </svg>
  `;
  return svg;
}
